                  ...
                  <div class="table-wrapper2">
                      <div class="title-and-btn">
                          ...
                          <div class="job-post-btn">
                              <router-link to="/company/post-job-ad" class="primry-btn-2">Create Advertisement</router-link>
                          </div>
                          <!-- <div class="job-post-btn">
                      ...
                      <table class="eg-table table job-list-table mb-0">
                          ...
                          <tbody>
                              ...
                              <tr v-else v-for="ad in jobAds" :key="ad.id">
                                  <td data-label="Ad Title">
                                      <div class="job-content">
                                          <h5>{{ ad.title }}</h5>
                                          <p><img src="assets/images/icon/calender2.svg" alt=""><span>Expiration:</span> {{ (ad.ends_at) ? ad.ends_at : 'No Deadline' }}</p>
                                          <!-- <InputSwitch v-model="ad.jobStatus" :modelValue="switchValue"   @update:modelValue="handleSwitchChange(job)" />
                                              Live -->
                                  ...
                                  <td data-label="Job Title">
                                        <div class="job-content">
                                          <h5>{{ ad.job.job_title }}</h5>
                                          <p class="ms-5"><img src="assets/images/icon/calender2.svg" alt=""><span>Job Expiration:</span> {{ (ad.ends_at) ? ad.ends_at : 'No Deadline' }}</p>
                                        </div>
                                  </td>
                                  ...
                                  <td data-label="Action">
                                      <ul class="action-btn-group2 nav nav-bar">
                                          <li>
                                              <router-link :to="getJobDetail(ad.job_key, ad.job_slug)">
                                          ...
                                          <li>
                                              <router-link :to="'/company/post-job-ad?ad_id='+ad.id">
...
methods: 
{
  ...
  fetchListing( pageId:any, searchJobTitle:any, searchJobDate:any)
      {
          ...
          axios.get(apiUrl + 'getCompanyAds?pageId=' + pageId + '&keyword=' + searchJobTitle + '&date=' + searchJobDate, {
          // headers: {
          ...
          .then(res => {
              console.log(res);
              this.jobAds = res.data.Listing; // Store fetched data
  ...
  updateAd(ad_id:any) {
      return {
            path: '/company/post-job-ad?ad_id='+ad_id
      };
  },  
},  
async mounted() {
  ...
  this.$store.dispatch('getCompanyAds', '');
...
watch: {
  companyAds() {
        this.jobAds = this.companyAds.Listing;
        this.totalPages =  this.companyAds.count;
    }
}
})
export default class CompanyJobAd extends Vue {}
</script>
